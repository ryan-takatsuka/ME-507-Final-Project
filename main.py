...
This is the main file that is run when the microcontroller is reset.
boot.py specifies that the file with the name 'main.py' will run following reboot.


@Authors: Ryan Takatsuka
"""
...
def hall_ISR(pin):
    """
    This function runs when there is an external interrupt due to the hall effect sensor.
    ...
    rotation_time = pyb.millis()


# setup the interrupt on the PA0 pin on the STM32 Discovery board
# interrupts = pyb.ExtInt(
#     pyb.Pin.cpu.A0, pyb.ExtInt.IRQ_FALLING, pyb.Pin.PULL_UP, hall_ISR)


# Method for finding the difference between elements in an array
def array_diff(in_array):
    """
    This function calculates the difference between list values.
    This is useful for determining timing intervals froms lists of times.
    This function is mainly used to determine the speed of certain aspects of the program
...
def timerTest():
    """
    This function prints out the times so that the program speed can be analyzed.
    This is mainly used right after a sequence of time sensitive commands, so that printing
    does not take up extra, unnecessary clock cycles.
...
while True:
    for x in range(width):			# For each column of image...
        for y in range(height):		# For each pixel in column...
            ...
            # Record the time it took to refresh the LEDs and to send the data
            # over SPI
            ...
            if pyb.elapsed_micros(start) > led_refresh_time:
                print("refresh time is too fast. Error on pixel #" +
                      str(y * width + x))
